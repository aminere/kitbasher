...
lm.utils.copy( lm.utils.ReactComponentHandler.prototype, {
	 ...
	 */
	_render: function() {
		// Amine patch for electron: ReactDOM is not guaranteed to return synchronously, so rewrote the render in an async friendly way		
		var scope = this;
		ReactDOM.render( this._getReactComponent(), this._container.getElement()[ 0 ], function () {
			scope._reactComponent = this;
			scope._originalComponentWillUpdate = scope._reactComponent.componentWillUpdate || function() {};
			scope._reactComponent.componentWillUpdate = scope._onUpdate.bind( scope );
			if( scope._container.getState() ) {
				scope._reactComponent.setState( scope._container.getState() );
			}
		});
	},
